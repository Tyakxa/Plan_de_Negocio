# PERSONAL REQUERIDO 
### Las personas se refieren al personal que interviene en la produccion con su mano de obra y conocimientos

|  | CANT | AREA | PRIORIDAD/FASE | ROL | DESCRIPCION | NOMBRE | REPORTA A | SUELDO MENSUAL |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |  |  | $126,068.00 |
| 1 | 1 | General | 1 | Director General | Direccion y administrador general | Tanya Rojas | --- | $35,358.00 |
| 2 | 1 | General | 1 | Director Tecnico | Direccion y administrador tecnico | Luis Esparza | 1 | $30,000.00 |
| 3 | 1 | General | 1 | Auxiliar contable | Asistente de direccion y administrador tecnico | Mireya Gonzalez | 1 | $20,000.00 |
| 4 | 1 | General | 1 | Recepcionista | Recibir clientes y atender telefonos | JCF | 3 | --- |
| 5 | 1 | General | 2 | Asistente General | Asistente de direccion y administrador tecnico | JCF | 1 | --- |
| 6 | 1 | General | 1 | RH | Asistente de direccion y administrador tecnico | JCF | 3 | --- |
| 7 | 1 | General | 1 | Limpieza | Se encarga de la limpieza del luegar  | --- | 3 | $10,710,00 |
| 8 | 1 | Soporte | 1 | Diagnostico y Recepcion | Recibir y documentar entrada de equipos fisica o por telefono | JCF | 2 | --- |
| 9 | 1 | Soporte | 1 | Tecnico | Reparar equipo nivel electronico | JCF | 2 | --- |
| 10 | 1 | Soporte | 1 | Tecnico | Reparar equipo general como formateos | JCF | 2 | --- |
| 11 | 1 | Soporte | 2 | Tecnico | Reparar impresoras | JCF | 2 | --- |
| 12 | 1 | Soporte | 2 | Tecnico | Reparar celulares | JCF | 2 | --- |
| 13 | 1 | Soporte | 1 | Mensajero | Recoleccion y entrega de equipos a domicilio | JCF | 2 | --- |
| 14 | 1 | TYAKXA (Desarrollo) | 1 | UX | Diseño de experiencia de usuario | JCF | 1 | --- |
| 15 | 1 | TYAKXA (Desarrollo) | 1 | UI | Diseño de interfaces | JCF | 1 | --- |
| 16 | 1 | TYAKXA (Desarrollo) | 1 | cyberseguridad | Monitoreo de servidores, respaldos y contencion de incidentes | JCF | 1 | --- |
| 17 | 1 | TYAKXA (Desarrollo) | 1 | Validacion | Validacion de tipos de dato | JCF | 1 | --- |
| 18 | 1 | TYAKXA (Desarrollo) | 1 | Tester | Pruebas de funcionalidad, seguridad e integracion | JCF | 1 | --- |
| 19 | 1 | Capacitacion | 2 | Asesor | Se revisar segun requerimiento | JCF | 1 | --- |
| 20 | 1 | General | 1 | Promotor | Promotor de Ventas de los 5 giros del negocio | JCF | 1 | --- |
| 21 | 1 | NegocioLocalMX(Marketing Digital) | 1 | Marketing | Arquitectore de estrategia de marketing e imagen  | JCF | 1 | --- |
| 22 | 1 | NegocioLocalMX(Marketing Digital) | 1 | Desarrollador web | Desarrollador digital | JCF | 1 | --- |
| 23 | 1 | NegocioLocalMX(Marketing Digital) | 1 | Branding | Encargado de Monitorear/Publicar en RRSS propias y clientes | JCF | 1 | --- |
| 24 | 1 | General | 1 | Cobrador | Encargado de caja y/o recepcion de pagos | JCF | 1 | --- |
---

## ORGANIGRAMA ORGANIZACIONAL

```
                        ┌─────────────────────────────────────┐
                        │   DIRECCIÓN GENERAL                 │
                        │   Tanya Rojas                       │
                        │   (Director General)                │
                        └──────────────┬──────────────────────┘
                                       │
                ┌──────────────────────┼──────────────────────┬─────────────────────┐
                │                      │                      │                     │
    ┌───────────▼──────────┐  ┌───────▼──────────┐  ┌───────▼──────────┐  ┌──────▼──────────┐
    │ DIRECCIÓN TÉCNICA    │  │ AUXILIAR         │  │ RECEPCIÓN        │  │ RECURSOS        │
    │ Luis Esparza         │  │ CONTABLE         │  │ (JCF)            │  │ HUMANOS (JCF)   │
    │ (Director Técnico)   │  │ Mireya González  │  └──────────────────┘  └─────────────────┘
    └──────────┬───────────┘  └──────────────────┘
               │
               │
    ┌──────────┴────────────────────────────┬──────────────────────────────┬─────────────────┐
    │                                       │                              │                 │
┌───▼─────────────────┐        ┌────────────▼────────────────┐  ┌─────────▼──────────────┐   │
│ ÁREA DE SOPORTE     │        │ ÁREA TYAKXA (DESARROLLO)    │  │ ÁREA DE CAPACITACIÓN   │   │
│                     │        │                             │  │                        │   │
│ ┌─────────────────┐ │        │ ┌─────────────────────────┐ │  │ ┌────────────────────┐ │   │
│ │ Diagnóstico y   │ │        │ │ Diseño UX/UI            │ │  │ │ Asesor/Instructor  │ │   │
│ │ Recepción (JCF) │ │        │ │ - UX (JCF)              │ │  │ │ (JCF) - Fase 2     │ │   │
│ └─────────────────┘ │        │ │ - UI (JCF)              │ │  │ └────────────────────┘ │   │
│                     │        │ └─────────────────────────┘ │  └────────────────────────┘   │
│ ┌─────────────────┐ │        │                             │                               │
│ │ Técnicos        │ │        │ ┌─────────────────────────┐ │                               │
│ │ - Electrónica   │ │        │ │ Calidad y Seguridad     │ │                               │
│ │   (JCF)         │ │        │ │ - Ciberseguridad (JCF)  │ │                    ┌─────────▼─────────┐
│ │ - General/      │ │        │ │ - Validación (JCF)      │ │                    │ SERVICIOS         │
│ │   Formateos(JCF)│ │        │ │ - Tester (JCF)          │ │                    │ GENERALES         │
│ │ - Impresoras    │ │        │ └─────────────────────────┘ │                    │                   │
│ │   (JCF) F2      │ │        └─────────────────────────────┘                    │ - Asistente       │
│ │ - Celulares     │ │                                                           │   General (JCF)   │
│ │   (JCF) F2      │ │                                                           │   Fase 2          │
│ └─────────────────┘ │                                                           │ - Limpieza        │
│                     │                                                           └───────────────────┘
│ ┌─────────────────┐ │
│ │ Mensajero (JCF) │ │
│ └─────────────────┘ │
└─────────────────────┘
```

## Resumen de Estructura Organizacional

### **NIVEL DIRECTIVO**
- **Director General:** Tanya Rojas ($65,358/mes)
- **Director Técnico:** Luis Esparza ($30,000/mes) - Coordina todas las áreas técnicas

### **ÁREA ADMINISTRATIVA** (Reportan a Dirección General)
- **Auxiliar Contable:** Mireya González ($20,000/mes)
- **Recepcionista:** JCF
- **Recursos Humanos:** JCF

### **ÁREAS OPERATIVAS** (Reportan a Dirección Técnica)

#### **1. Soporte y Reparaciones (6 personas)**
- Diagnóstico y Recepción (JCF)
- Técnico Electrónica (JCF)
- Técnico General/Formateos (JCF)
- Técnico Impresoras (JCF) - Fase 2
- Técnico Celulares (JCF) - Fase 2
- Mensajero (JCF)

#### **2. TYAKXA - Desarrollo (5 personas)**
- **Diseño:**
  - UX (JCF)
  - UI (JCF)
- **Calidad y Seguridad:**
  - Ciberseguridad (JCF)
  - Validación (JCF)
  - Tester (JCF)

#### **3. Capacitación (1 persona - Fase 2)**
- Asesor/Instructor (JCF)

### **SERVICIOS GENERALES**
- Asistente General (JCF) - Fase 2
- Personal de Limpieza ($10,710/mes)

---

## Datos Clave

- **Total de personal:** 19 personas
- **Personal con sueldo fijo:** 4 personas
- **Programa JCF:** 15 personas
- **Nómina mensual total:** $126,068.00

**Leyenda:**
- JCF = Jóvenes Construyendo el Futuro
- F2 = Fase 2 (contratación posterior)

**Coordinadores:**
1. Tanya Rojas (Director General)
2. Luis Esparza (Director Técnico)
3. Mireya González (Auxiliar Contable)

